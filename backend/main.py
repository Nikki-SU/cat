...
from routers import (
    ...
    settings as settings_router, note_image, note_ocr, sync_v2, pairing
)
...
@app.middleware("http")
async def device_id_middleware(request, call_next):
    """Set device_id context for change tracking"""
    from services.hub_manager import get_hub_manager
    manager = get_hub_manager()
    device_id = manager.get_device_id()
...
app.include_router(note_ocr.router, prefix=settings.API_PREFIX)
